...
async function loadAllData() {
    ...
    if (maternity) {
        document.getElementById('mb-eyebrow').value = maternity.heroEyebrow || '';
        document.getElementById('mb-line1').value = maternity.heroLine1 || '';
        document.getElementById('mb-line2').value = maternity.heroLine2 || '';
        document.getElementById('mb-subtext').value = maternity.heroSubtext || '';
        if (maternity.heroImage) document.getElementById('mb-hero-image-preview').innerHTML = `<img src="${maternity.heroImage}" style="max-width:300px;margin-top:8px;border-radius:4px;">`;
        document.getElementById('mb-intro-1').value = maternity.introPara1 || '';
        document.getElementById('mb-intro-2').value = maternity.introPara2 || '';
        const mbGal = maternity.galleryImages || [];
        [1, 2, 3].forEach(i => {
            if (mbGal[i - 1]) document.getElementById(`mb-gallery-${i}-preview`).innerHTML = `<img src="${mbGal[i - 1]}" style="max-width:200px;margin-top:8px;border-radius:4px;">`;
        });
        document.getElementById('mb-why1-title').value = maternity.why1Title || '';
        document.getElementById('mb-why1-desc').value = maternity.why1Desc || '';
        document.getElementById('mb-why2-title').value = maternity.why2Title || '';
        document.getElementById('mb-why2-desc').value = maternity.why2Desc || '';
        document.getElementById('mb-why3-title').value = maternity.why3Title || '';
        document.getElementById('mb-why3-desc').value = maternity.why3Desc || '';
        document.getElementById('mb-testimonial-quote').value = maternity.testimonialQuote || '';
...
// ─── Maternity Page ─────────────────────────────────────────────────────────
async function saveMaternityPage() {
    showToast('Saving maternity page…');
    const existing = await dbGet('maternity_page') || {};

    let heroImage = existing.heroImage || '';
    const heroFile = document.getElementById('mb-hero-image');
    if (heroFile.files[0]) {
        heroImage = await uploadImage(heroFile.files[0]);
        document.getElementById('mb-hero-image-preview').innerHTML = `<img src="${heroImage}" style="max-width:300px;margin-top:8px;border-radius:4px;">`;
    }

    const galleryImages = existing.galleryImages ? [...existing.galleryImages] : ['', '', ''];
    for (const i of [1, 2, 3]) {
        const input = document.getElementById(`mb-gallery-${i}`);
        if (input.files[0]) {
            const url = await uploadImage(input.files[0]);
            galleryImages[i - 1] = url;
            document.getElementById(`mb-gallery-${i}-preview`).innerHTML = `<img src="${url}" style="max-width:200px;margin-top:8px;border-radius:4px;">`;
        }
    }

    const data = {
        heroEyebrow: document.getElementById('mb-eyebrow').value.trim(),
        heroLine1: document.getElementById('mb-line1').value.trim(),
        heroLine2: document.getElementById('mb-line2').value.trim(),
        heroSubtext: document.getElementById('mb-subtext').value.trim(),
        heroImage,
        introPara1: document.getElementById('mb-intro-1').value.trim(),
        introPara2: document.getElementById('mb-intro-2').value.trim(),
        galleryImages,
        why1Title: document.getElementById('mb-why1-title').value.trim(),
        why1Desc: document.getElementById('mb-why1-desc').value.trim(),
        why2Title: document.getElementById('mb-why2-title').value.trim(),
        why2Desc: document.getElementById('mb-why2-desc').value.trim(),
        why3Title: document.getElementById('mb-why3-title').value.trim(),
        why3Desc: document.getElementById('mb-why3-desc').value.trim(),
        testimonialQuote: document.getElementById('mb-testimonial-quote').value.trim(),
        testimonialAuthor: document.getElementById('mb-testimonial-author').value.trim(),
        ctaHeading: document.getElementById('mb-cta-heading').value.trim(),
        ctaSubtext: document.getElementById('mb-cta-subtext').value.trim()
    };
